...
async def test_send_notification_task_success(mock_email_provider, notification_dto):
    ...
    mock_email_provider.send = AsyncMock(
        return_value=MagicMock(success=True, message_id="test_id")
    )
    
    with patch("src.notification_service.router.AVAILABLE_PROVIDERS", 
...
async def test_send_notification_task_failure(mock_email_provider, notification_dto):
    ...
    mock_email_provider.send = AsyncMock(
        return_value=MagicMock(success=False, error="Send failed")
    )
