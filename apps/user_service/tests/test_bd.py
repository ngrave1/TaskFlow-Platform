...
class TestGetUserByEmail:
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_email_case_sensitive(self, async_session, test_user):
        user_lower = await get_user_by_email(async_session, test_user["email"].lower())
        assert user_lower is not None
        
        user_upper = await get_user_by_email(async_session, test_user["email"].upper())
...
class TestGetUserById:    
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_invalid_id(self, async_session):
        user = await get_user_by_id(async_session, 0)
        assert user is None
...
class TestPasswordHashingIntegration:
    ...
    async def test_password_hashing_roundtrip(self, async_session):
        from src.user_service.orm_utils import create_user, session_add, get_user_by_email
        ...
        from src.user_service.password_utils import check_password
        is_valid = check_password(plain_password, db_user.password)
        assert is_valid is True
        is_invalid = check_password("WrongPassword", db_user.password)
        ...
        async with async_session_factory() as session1:
            async with async_session_factory() as session2:
                ...
                result = await session2.execute(
                    select(Users).where(Users.email == "session1@test.com")
                )
                user_in_session2 = result.scalar_one_or_none()
                if user_in_session2 is None:
